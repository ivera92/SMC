...
create table ASIGNATURA_COMPETENCIA (
   ID_AC                int                  identity,
   COD_ASIGNATURA_AC    char(7)              not null,
   ID_COMPETENCIA_AC    int                  not null,
   NIVEL_DOMINIO_AC     char(3)              null,
   constraint PK_ASIGNATURA_COMPETENCIA primary key (COD_ASIGNATURA_AC, ID_COMPETENCIA_AC)
)
go
...
create table EVALUACION (
   ...
   COD_ASIGNATURA_EVALUACION char(7)              not null,
...
create table RELATIONSHIP_19 (
   ...
   COD_ASIGNATURA_AA    char(7)              not null,
   constraint PK_RELATIONSHIP_19 primary key (RUT_ALUMNO_AA, COD_ASIGNATURA_AA)
)
go
...
alter table RELATIONSHIP_19
   add constraint FK_RELATION_CURSA_ASIGNATU foreign key (COD_ASIGNATURA_AA)
      references ASIGNATURA (COD_ASIGNATURA)
go
